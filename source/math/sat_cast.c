...
size_t dark_sat_ucast_zu(const uintmax_t u_)
{
    //u_

    return DARK_MIN(u_, SIZE_MAX);
}
...
intmax_t dark_sat_ucast_imax(const uintmax_t u_)
{
    //u_

    return DARK_MIN(u_, INTMAX_MAX);
}
...
size_t dark_sat_icast_zu(const intmax_t i_)
{
    //i_
